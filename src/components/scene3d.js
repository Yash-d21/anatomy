...
export class Scene3d {
  ...
  attached() {
    ...
    this.transformControl.setMode('rotate'); // Rotation mode only
    this.transformControl.setSpace('world'); // Default to world space
    this.transformControl.size = 1.5; // Larger for better visibility

    // Position model to the right side of the screen
    this.modelGroup.position.set(25, 0, 0);
    // Update camera/controls to look at the offset position
    this.controls.target.set(25, 0, 0);
    this.controls.update();

    this.transformControl.addEventListener('dragging-changed', (event) => {
      this.controls.enabled = !event.value;
      if (!event.value) this.lastTransformDragTime = Date.now();
    });
    ...
    this.lastTransformDragTime = 0;

    // Event listeners
    this.onMouseDownBound = this.onMouseDown.bind(this);
    this.onKeyDownBound = this.onKeyDown.bind(this);
    this.renderer.domElement.addEventListener('mousedown', this.onMouseDownBound, false);
    window.addEventListener('keydown', this.onKeyDownBound, false);
  }

  onMouseDown(event) {
    // Ignore if we just finished dragging the transform control
    if (Date.now() - this.lastTransformDragTime < 100) return;
    ...
    // Intersect objects in the model group
    const intersects = this.raycaster.intersectObjects(this.modelGroup.children, true);

    if (intersects.length > 0) {
      // If we hit any part of the model, select the WHOLE model group
      this.transformControl.attach(this.modelGroup);
    } else {
      // Check if we hit the gizmo itself to avoid detaching while interacting
  ...
  onKeyDown(event) {
    switch (event.key.toLowerCase()) {
      case 'r': // Rotation mode
        ...
        break;
      // case 't': // Translate mode (future proofing)
      //   this.transformControl.setMode('translate');
      //   break;
      case 'l': // Toggle local/world space
  ...
  declareOrbitControls() {
    ...
    controls.dampingFactor = 0.05; // Smoothness factor
    controls.screenSpacePanning = true; // Pan direction corresponds to screen 
  ...
  loadfromurl(myurl) {
    ...
    this.loader.load(
      ...
      function (gltf) {
        ...
        // Add to modelGroup instead of scene to support grouped rotation
        window.that.modelGroup.add(gltf.scene);
        window.that.progresstxt = '';
        // Auto-attach gizmo once initially loaded
        window.that.transformControl.attach(window.that.modelGroup);
  ...
  animate() {
    window.that.renderer.setAnimationLoop(function () {
      if (window.that.controls) window.that.controls.update(); // Update controls for damping
